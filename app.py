...
class Config:
    ...
    def __init__(self):
        ...
        self.position_keywords = {
            'PK10': {
                '冠军': ['冠军', '第1名', '第一名', '前一', '冠 军', '冠　军', '1st', '第一位'],
                '亚军': ['亚军', '第2名', '第二名', '亚 军', '亚　军', '2nd', '第二位'],
                '季军': ['季军', '第3名', '第三名', '季 军', '季　军', '3rd', '第三位'],
                '第四名': ['第四名', '第4名', '第四位', '4th'],
                '第五名': ['第五名', '第5名', '第五位', '5th'],
                '第六名': ['第六名', '第6名', '第六位', '6th'],
                '第七名': ['第七名', '第7名', '第七位', '7th'],
...
class ContentParser:
    ...
    def extract_basic_directions(content, config):
        ...
        # 检查所有彩种的位置关键词
        for lottery_type, positions in config.position_keywords.items():
        ...
        for direction, patterns in config.direction_patterns.items():
            for pattern in patterns:
                ...
                if (pattern_lower == content_lower or 
                    ...
                    # 如果检测到位置信息，组合位置和方向
                    if position_detected and detected_position:
                        directions.append(f"{detected_position}-{direction}")
                    else:
    ...
    @staticmethod
    def enhanced_extract_position_from_content(content, play_category, lottery_type, config):
        """增强版位置提取 - 从内容和玩法中综合提取位置"""
        content_str = str(content).strip()
        play_str = str(play_category).strip()
        
        # 方法1：从玩法分类中提取
        position_from_play = ContentParser.extract_position_from_play_category(play_category, lottery_type, config)
        ...
        if '|' in content_str:
            if lottery_type == 'PK10':
                bets_by_position = ContentParser.parse_pk10_vertical_format(content_str)
                for position in bets_by_position:
                    if bets_by_position[position]:
                        return position
            elif lottery_type == '3D':
                bets_by_position = ContentParser.parse_3d_vertical_format(content_str)
                for position in bets_by_position:
                    if bets_by_position[position]:
                        return position
...
class WashTradeDetector:
    ...
    def enhanced_extract_direction_with_position(self, content, play_category, lottery_type):
        ...
        try:
            ...
            # 🎯 增强位置提取：从内容和玩法中综合提取
            position = self.content_parser.enhanced_extract_position_from_content(
            ...
            if position and position != '未知位置':
                # 检查方向是否已经包含位置信息
                if '-' in main_direction:
                    return main_direction  # 已经包含位置，直接返回
                else:
